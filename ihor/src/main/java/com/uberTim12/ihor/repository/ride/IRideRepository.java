...
public interface IRideRepository extends JpaRepository<Ride, Integer> {
    ...
    @Query("select r from Ride r left join r.passengers p where (r.driver.id = ?1 or p.id = ?1) and r.startTime between ?2 and ?3")
    Page<Ride> findAllInRangeForUser(Integer id, LocalDateTime start, LocalDateTime end, Pageable pageable);

    @Query("select r from Ride r left join r.passengers p where r.driver.id = ?1 or p.id = ?1")
